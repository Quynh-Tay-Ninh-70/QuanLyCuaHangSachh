...
public class NXBDAO {
    ...
    public NXBDAO() throws IOException {
     ...
     public ArrayList<NXBPOJO> loadDatabase() throws Exception
    {
        ArrayList<NXBPOJO> dsnxb = new ArrayList<>();
        try {
            ResultSet rs = connect.Select("nhaxuatban");
            while(rs.next())
            {
                NXBPOJO nxb = new  NXBPOJO(rs.getString("MaNXB"),rs.getString("TenNXB"),rs.getString("DiaChiNXB"),rs.getString("SDT"));     
                dsnxb.add(nxb);
            }
            rs.close();
