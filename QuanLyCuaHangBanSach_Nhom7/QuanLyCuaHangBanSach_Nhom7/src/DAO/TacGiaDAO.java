...
public class TacGiaDAO {
    ...
    public TacGiaDAO() throws IOException {
     ...
     public ArrayList<TacGiaPOJO> loadDatabase() throws Exception
    {
        ArrayList<TacGiaPOJO> dstacgia = new ArrayList<>();
        try {
            ResultSet rs = connect.Select("tacgia");
            while(rs.next())
            {
                TacGiaPOJO tg = new  TacGiaPOJO(rs.getString("MaTG"),rs.getString("TenTG"),rs.getString("DiaChi"),rs.getString("SDT"));     
                dstacgia.add(tg);
            }
            rs.close();
